...
var ajaxUtil = function() {
	...
	var GET = function(requestFile, callBackFunc, isXML) {
	...
	// Post something and return it.
	var POST = function(postURL, postData, callBackFunc) {
		//msg.set("Send Ajax POST.");
		var data = encodeURI(postData);
		r.open("POST", postURL, true);
		r.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		r.onreadystatechange = function() {
			if (r.readyState == 4 && statusCodeTest(r.status)) {
				// msg.set("Got Ajax Response");
				callBackFunc(r.responseText);
			}
		}
